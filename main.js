/* Obsidian Bases Image Gallery Community plugin: https://github.com/ghyatt/obsidian-base-image-gallery */
...
// src/build-lightbox.ts
var lightbox = (gallery, imagesList, app) => {
  if (import_obsidian.Platform.isDesktop)
    globalSearchBtn(gallery, imagesList, app);
...
var formatDate = (ms) => Number.isFinite(ms) ? new Date(ms).toLocaleDateString() : "";
var infoPanel = (gallery, imagesList) => {
  gallery.addEventListener("lgInit", (event) => {
    ...
    const currentDimensions = () => {
      const img = galleryInstance.outer.find(".lg-current .lg-image").get();
      return img && img.naturalWidth ? `${img.naturalWidth} \xD7 ${img.naturalHeight}` : "";
    };
    const update = () => {
...
var closeOnDoubleClick = (gallery) => {
  gallery.addEventListener("lgInit", (event) => {
    const galleryInstance = event.detail.instance;
    galleryInstance.outer.on("dblclick", (e) => {
      const target = e.target;
      if (target.closest(".lg-toolbar, .lg-prev, .lg-next, .lg-thumb-outer"))
        return;
...
// src/bases-view.ts
var VIEW_TYPE = "image-gallery";
var VALID_EXTENSIONS = ["jpeg", "jpg", "gif", "png", "webp", "tiff", "tif"];
var MAX_ENTRIES = 500;
var GalleryBasesView = class extends import_obsidian2.BasesView {
  ...
  onDataUpdated() {
    ...
    if (images.length === 0) {
      render_error_default(this.viewContainerEl, "No images match this view. Filter the Base to image files (jpeg, jpg, gif, png, webp, tiff, tif).");
      return;
    }
    const visible = images.slice(0, MAX_ENTRIES);
    if (settings.type === "vertical") {
      this.gallery = build_vertical_default(this.viewContainerEl, visible, settings);
    } else {
      this.gallery = build_horizontal_default(this.viewContainerEl, visible, settings);
    }
    ...
    this.lightbox = build_lightbox_default(this.gallery, visible, this.app);
  }
  onunload() {
    this.destroyGallery();
    this.viewContainerEl.empty();
  ...
  readSettings() {
    ...
    return {
      ...
      columns: import_obsidian2.Platform.isDesktop ? desktopColumns : mobileColumns,
  ...
  getImagesList() {
    ...
    for (const entry of this.data.data) {
      ...
      if (!(file instanceof import_obsidian2.TFile))
        continue;
      if (!VALID_EXTENSIONS.includes(file.extension.toLowerCase()))
        continue;
      images.push({
        name: file.name,
...
// src/main.ts
var BasesImageGallery = class extends import_obsidian3.Plugin {
  onload() {
